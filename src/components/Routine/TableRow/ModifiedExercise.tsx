...
export default function ModifiedExercise({ id, name, reps, series, setOpen, routineOrWarmUp, setLoader, setRoutineAdmin, setWarmUpAdmin, caseResolve }: ModifiedExerciseProps) {
    ...
    return (
        <div style={{ border: 'solid, black, 2px', position: 'absolute', top: '50%', right: '50%', background: 'white' }}>
            ...
            <button onClick={() => modifiedExercise({ id, inputs, routineOrWarmUp, setOpen, setLoader, setRoutineAdmin, setWarmUpAdmin, caseResolve })}> Modificar </button>
        </div>
    )
}